...
AS
	SELECT 
      ...
      ltrim(rtrim( [FACT].[CVE_CLPV])) [U_MasterIDCustomer],
       ...
       [D].[UNI_VENTA] [unitMsr],
       '' [statusOfMaterial]
FROM [$(CICDSaeBD)].[dbo].[FACTP01] [FACT]
    INNER JOIN  [$(CICDSaeBD)].[dbo].[PAR_FACTP01] [D]
        ON [D].[CVE_DOC] = [FACT].[CVE_DOC] COLLATE DATABASE_DEFAULT
		INNER JOIN [$(CICDSaeBD)].[dbo].[INVE01] [m] ON [m].[CVE_ART] = [D].[CVE_ART] COLLATE DATABASE_DEFAULT
    LEFT JOIN [$(CICDSaeBD)].[dbo].[CLIE01] [CLIE]
        ON [CLIE].[CLAVE] = [FACT].[CVE_CLPV]  COLLATE DATABASE_DEFAULT
    LEFT JOIN [$(CICDSaeBD)].[dbo].[VEND01] [VEND]
