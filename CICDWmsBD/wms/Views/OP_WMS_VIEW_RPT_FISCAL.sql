...
CREATE VIEW [wms].[OP_WMS_VIEW_RPT_FISCAL]
AS
SELECT     (SELECT     CLIENT_NAME
                      ...
                      B.CURRENT_LOCATION, ISNULL(C.VOLUME_FACTOR, 0.1) AS VOLUMEN, ISNULL(C.VOLUME_FACTOR, 0.1) * A.QTY AS TOTAL_VOLUMEN, ISNULL(C.WEIGTH, 0) 
                      AS WEIGTH, ISNULL(C.WEIGTH, 0) * A.QTY AS TOTAL_WEIGTH, B.LAST_UPDATED_BY
...
WHERE     (B.REGIMEN = 'FISCAL') AND (A.QTY > 0)