...
-- Autor:				Denis Arango
-- Fecha de Creacion: 	23-Nov-2022
-- Descripcion:			Se modifica la validacion para que no se pase de la cantidad del documento para que tome los registros de la trans
...
*/
-- =============================================
CREATE PROCEDURE [wms].[OP_WMS_SP_AGREGA_SKU_LICENCIA] (@pLICENSE_ID NUMERIC(18, 0),
@pBARCODE VARCHAR(25),
@pQTY NUMERIC(18, 4),
@pLAST_LOGIN VARCHAR(25),
  ...
  BEGIN TRY
    BEGIN
      ...
      BEGIN
        ...
        -- ------------------------------------------------------------------------------------
        SELECT
          @ALLOWS_TO_STORE_MORE_QUANTITY = CAST([VALUE] AS INT)
        ...
        BEGIN
          ...
          BEGIN
            ...
            SELECT
              @QTY_TRANS_MATERIAL = SUM([T].[QUANTITY_UNITS])
            FROM [wms].[OP_WMS_LICENSES] [L]
            INNER JOIN [wms].[OP_WMS_POLIZA_HEADER] [PH]
              ON ([PH].[CODIGO_POLIZA] = [L].[CODIGO_POLIZA])
            INNER JOIN [wms].[OP_WMS_POLIZA_DETAIL] [PD]
            ...
            -- ------------------------------------------------------------------------------------
            IF (@QTY_TOTAL_IN_TRANS > @MAX_QTY
              AND @ALLOWS_TO_STORE_MORE_QUANTITY = 0)
            BEGIN
              SELECT
                @ErrorCode = 1111;
              RAISERROR (
        ...
        BEGIN
          ...
          SELECT
            [RD].[UNIT]
           ,SUM(ISNULL([RD].[QTY], 0)) * ISNULL([UMM].[QTY], 1) [QTY] INTO [#MAX_QTY]
          ...
          BEGIN
            SELECT
              @MAX_QTY = @MAX_QTY - ISNULL(SUM([T].[QUANTITY_UNITS]), 0)
            FROM wms.OP_WMS_TRANS T
			WHERE T.MATERIAL_CODE = @pMATERIAL_ID
        ...
        BEGIN
          ...
          INSERT INTO @STATUS_TB ([RESULTADO],
          [MENSAJE],
          [CODIGO],
          [STATUS_ID])
          ...
          ELSE
          BEGIN
            SET @TONE_AND_CALIBER_ID = (SELECT TOP 1
                [STATUS_ID]
              FROM @STATUS_TB);
        ...
        SELECT
          @pAcuerdoComercial = [P].[ACUERDO_COMERCIAL]
        ...
        BEGIN
          SELECT
            @ERROR = 'No puede ingresar un producto relacionado a un masterpack, ingresado';
        ...
        BEGIN
          ...
          BEGIN
            SELECT
              @ERROR = 'El VIN que ingreso ya existe: ' + @VIN;
        ...
        BEGIN
          ...
          BEGIN
            SELECT
              @ERROR = 'El tono o calibre es diferente a la ingresada anteriormente.';
        ...
        BEGIN
          SELECT
            @ERROR = 'No puede ingresar el mismo producto con diferente estado en la licencia.';
        ...
        BEGIN
          --
          INSERT INTO [wms].[OP_WMS_INV_X_LICENSE] ([LICENSE_ID],
          [MATERIAL_ID],
          [QTY],
          [LAST_UPDATED_BY],
          [LAST_UPDATED],
          ...
          [IDLE],
          [PROJECT_ID])
            VALUES (@pLICENSE_ID, @pMATERIAL_ID, @pQTY, @pLAST_LOGIN, CURRENT_TIMESTAMP, (SELECT * FROM [wms].[OP_WMS_FUNC_GETMATERIAL_DESC](@pBARCODE, (SELECT [CLIENT_OWNER] FROM [wms].[OP_WMS_LICENSES] WHERE [LICENSE_ID] = @pLICENSE_ID))), ISNULL((SELECT [VOLUME_FACTOR] FROM [wms].[OP_WMS_MATERIALS] WHERE [MATERIAL_ID] = @pMATERIAL_ID), 0), ISNULL((SELECT [WEIGTH] FROM [wms].[OP_WMS_MATERIALS] WHERE [MATERIAL_ID] = @pMATERIAL_ID), 0), @pComments, @pSERIAL_ID, @pBARCODE, @pAcuerdoComercial, @pSTATUS, @DATE_EXPIRATION, @BATCH, @pQTY, @VIN, @HANDEL_SERIEAL, @STATUS_ID, @TONE_AND_CALIBER_ID, @LOCKED_BY_INTERFACES, @ENTERED_MEASUREMENT_UNIT, @ENTERED_MEASUREMENT_UNIT_QTY, @ENTERED_MEASUREMENT_UNIT_CONVERSION_FACTOR, @CODE_SUPPLIER, @NAME_SUPPLIER, @IDLE, @PROJECT_ID);
          ...
          BEGIN
            ...
            INSERT INTO [wms].[OP_WMS_INVENTORY_RESERVED_BY_PROJECT] ([PROJECT_ID],
            [PK_LINE],
            [LICENSE_ID],
            [MATERIAL_ID],
            [MATERIAL_NAME],
            ...
            [DATE_EXPIRATION],
            [STATUS_CODE])
              VALUES (@PROJECT_ID,       -- PROJECT_ID - uniqueidentifier
            ...
            INSERT INTO [wms].[OP_WMS_LOG_INVENTORY_RESERVED_BY_PROJECT] ([TYPE_LOG],
            [PROJECT_ID],
            [PK_LINE],
            [LICENSE_ID],
            [MATERIAL_ID],
            ...
            [CREATED_BY],
            [CREATED_DATE])
              VALUES ('INSERT',              -- TYPE_LOG - varchar(20)
