...
-- Modificacion 10/9/2017 @ NEXUS-Team Sprint ewms
					-- rodrigo.gomez
...
EXEC [wms].[OP_WMS_SP_MARK_TRANSFER_REQUEST_AS_SEND_TO_ERP] 
	@RECEPTION_DOCUMENT_ID = 10000, -- int
	@POSTED_RESPONSE = 'Exito', -- varchar(500)
	@ERP_REFERENCE = '1', -- varchar(50)
	@OWNER = 'autovanguard' -- varchar(50)
*/
-- =============================================

CREATE PROCEDURE [wms].[OP_WMS_SP_MARK_TRANSFER_REQUEST_AS_SEND_TO_ERP](
...
BEGIN
	...
	BEGIN TRY
		...
		-- ------------------------------------------------------------------------------------
		IF NOT EXISTS (SELECT * FROM [wms].[OP_WMS_ERP_RECEPTION_DOCUMENT_DETAIL] WHERE [IS_POSTED_ERP] <> 1 AND [ERP_RECEPTION_DOCUMENT_HEADER_ID] = @RECEPTION_DOCUMENT_ID)
		BEGIN
		...
		LEFT JOIN [wms].[OP_WMS_MATERIAL_PROPERTY_BY_WAREHOUSE] [MW] ON [M].[MATERIAL_ID] = [MW].[MATERIAL_ID] AND [MW].[WAREHOUSE_ID] = (ISNULL([D].[WAREHOUSE_CODE], [W].[WAREHOUSE_ID]))
		LEFT JOIN [wms].[OP_WMS_MATERIAL_PROPERTY] [MP] ON [MP].[MATERIAL_PROPERTY_ID] = [MW].[MATERIAL_PROPERTY_ID] AND [MP].[NAME] = 'EXPLODE_IN_RECEPTION'
