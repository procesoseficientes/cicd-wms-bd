...
-- Ejemplo de Ejecucion:
		EXEC  [wms].OP_WMS_GET_ERP_SALES_ORDER_DETAIL_BY_ORDERS_CHANNEL_MODERN 
			@DOC_NUMS = '14022|14159|14166|14164', 
			@EXTERNAL_SOURCE_IDS = '1|1|1|1|', 
			@CODE_WAREHOUSE = 'BODEGA_SPS',
			@START_DATE = '2011-09-26 00:00:00.00',
...
*/
-- =============================================
CREATE PROCEDURE [wms].[OP_WMS_GET_ERP_SALES_ORDER_DETAIL_BY_ORDERS_CHANNEL_MODERN_1072021]
    (
...
AS
  ...
  --                      + ''',
      ...
      ,SOD.[statusOfMaterial] 
		 FROM
			wms.ERP_VIEW_SALES_ORDER_DETAIL SOD WITH(NOLOCK)
		INNER JOIN [wms].OP_WMS_COMPANY OWC WITH(NOLOCK) ON (
		OWC.CLIENT_CODE COLLATE DATABASE_DEFAULT = SOD.U_OwnerSKU COLLATE DATABASE_DEFAULT
		AND OWC.EXTERNAL_SOURCE_ID = @EXTERNAL_SOURCE_ID) --CAST(@EXTERNAL_SOURCE_ID AS VARCHAR))                      
		INNER JOIN [wms].OP_WMS_MATERIALS M WITH(NOLOCK) ON (
		SOD.ItemCode COLLATE DATABASE_DEFAULT = [M].[ITEM_CODE_ERP] COLLATE DATABASE_DEFAULT 
		AND SOD.U_OwnerSKU COLLATE DATABASE_DEFAULT= M.CLIENT_OWNER COLLATE DATABASE_DEFAULT )
		INNER JOIN #SALES_ORDERS SOTMP WITH(NOLOCK) ON (
		SOD.DOCNUM COLLATE DATABASE_DEFAULT = SOTMP.SALE_ORDER_ID COLLATE DATABASE_DEFAULT 
		AND SOTMP.EXTERNAL_SOURCE_ID = @EXTERNAL_SOURCE_ID) --CAST(@EXTERNAL_SOURCE_ID AS VARCHAR))
		...
		LEFT JOIN [wms].[OP_WMS_UNIT_MEASUREMENT_BY_MATERIAL] [UMM] ON [M].[MATERIAL_ID] COLLATE DATABASE_DEFAULT = [UMM].[MATERIAL_ID] COLLATE DATABASE_DEFAULT
                                                              AND [UMM].[MEASUREMENT_UNIT] COLLATE DATABASE_DEFAULT = SOD.unitMsr COLLATE DATABASE_DEFAULT
		WHERE 
			SOD.DocDate BETWEEN CAST(@START_DATE AS VARCHAR) AND  CAST(@END_DATE AS VARCHAR)
			AND CEILING((SOD.Quantity * ISNULL(UMM.QTY, 1)) - ISNULL(DD.QTY, 0)) > 0 
