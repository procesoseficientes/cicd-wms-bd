 ...
 */
-- =============================================
CREATE PROCEDURE [wms].[OP_WMS_SP_INSERT_TASK_EXPLOSION] (@TASK_SUBTYPE VARCHAR(25)
...
BEGIN
  ...
  WHERE [CODIGO_POLIZA] = @CODIGO_POLIZA_SOURCE;

  INSERT INTO [wms].[OP_WMS_TASK_LIST] ([TASK_TYPE]
  , [TASK_SUBTYPE]
  , [TASK_OWNER]
  , [TASK_ASSIGNEDTO]
  , [TASK_COMMENTS]
  ...
  , [ORDER_NUMBER])
    VALUES ('EXPLOSION MANUAL', @TASK_SUBTYPE, @TASK_OWNER, @TASK_ASSIGNEDTO, @TASK_COMMENTS, @REGIMEN, 1, 0, 0, 0, 1, @CLIENT_OWNER, @CLIENT_NAME, @DOC_ID_SOURCE, GETDATE(), 0, 0, 0, '', 0, @CODIGO_POLIZA_SOURCE, @PRIORITY, CASE WHEN @SEND_TO_ERP = 1 THEN 1 ELSE @IS_FROM_ERP END, @LOCATION_SPOT_TARGET, @ORDER_NUMBER);
  SET @ID = SCOPE_IDENTITY();
  --

  UPDATE [wms].[OP_WMS_TASK_LIST]
  SET [TASK_COMMENTS] = 'TAREA DE EXPLOSION MANUAL NO. '
  + CAST(@ID AS VARCHAR)
