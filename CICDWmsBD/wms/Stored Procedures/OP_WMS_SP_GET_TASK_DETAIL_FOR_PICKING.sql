...
WHERE
	...
	AND ISNULL(A.HANDLED_PER_CHANNEL, 0) = 0
GROUP BY
    [A].[WAVE_PICKING_ID]
   ,[A].[TASK_ASSIGNEDTO]
   ,[A].[MATERIAL_ID]
   ...
   ,[A].[WAVE_PICKING_ID]
   ,[TASK_ASSIGNEDTO] [ASSIGNED_TO]
   ,CASE [SS].[FOR_FORKLIFT]
		WHEN 1 THEN 'MONTACARGAS'
		ELSE 'PISO'
...
LEFT JOIN [wms].[OP_WMS_WAREHOUSES] [W] ON [A].[WAREHOUSE_SOURCE] = [W].[WAREHOUSE_ID]
LEFT JOIN [wms].[OP_WMS_SHELF_SPOTS] [SS] ON [SS].[LOCATION_SPOT] = [A].LOCATION_SPOT_SOURCE
WHERE
    [A].[WAVE_PICKING_ID] = @WAVE_PICKING_ID
    AND [A].[TASK_TYPE] = 'TAREA_PICKING'
    AND (
	...
	AND ISNULL(A.HANDLED_PER_CHANNEL, 0) = 1
GROUP BY
	[A].[SERIAL_NUMBER]
   ,[A].[WAVE_PICKING_ID]
   ,[A].[TASK_ASSIGNEDTO]
