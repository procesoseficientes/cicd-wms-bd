...
BEGIN
	...
	INNER JOIN [$(CICDSaeBD)].dbo.INVE01 Inventario ON Inventario.CVE_ART COLLATE DATABASE_DEFAULT = [M].BARCODE_ID  
    LEFT JOIN [$(CICDSaeBD)].dbo.MULT01 MultiAlmacen ON Inventario.CVE_ART = MultiAlmacen.CVE_ART  AND [MultiAlmacen].[CVE_ALM]  = [WT].[ERP_WAREHOUSE] 
	LEFT JOIN [$(CICDSaeBD)].dbo.vConsumoPromedio [PROM] ON ([PROM].CVE_ART COLLATE DATABASE_DEFAULT = M.[BARCODE_ID] COLLATE DATABASE_DEFAULT AND [PROM].[ALMACEN]  = [WT].[ERP_WAREHOUSE] )
	WHERE
