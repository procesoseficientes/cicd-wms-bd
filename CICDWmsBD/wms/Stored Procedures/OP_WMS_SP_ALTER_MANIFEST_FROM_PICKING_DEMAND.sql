...
-- Ejemplo de Ejecucion:
				EXEC [wms].[OP_WMS_SP_ALTER_MANIFEST_FROM_PICKING_DEMAND]
					@VEHICLE_CODE = 32
					,@LOGIN = 'BETO'
...
*/
-- =============================================
CREATE PROCEDURE wms.OP_WMS_SP_ALTER_MANIFEST_FROM_PICKING_DEMAND (@VEHICLE_CODE INT
, @LOGIN VARCHAR(50)
, @XML XML)
AS
BEGIN
  ...
  BEGIN TRY
    ...
    BEGIN
      ...
      -- ------------------------------------------------------------------------------------
      INSERT INTO [wms].[OP_WMS_MANIFEST_HEADER] ([DRIVER]
      , [VEHICLE]
      , [DISTRIBUTION_CENTER]
      , [CREATED_DATE]
      , [STATUS]
    ...
    -- ------------------------------------------------------------------------------------
    INSERT INTO [wms].[OP_WMS_MANIFEST_DETAIL] ([MANIFEST_HEADER_ID]
    , [CODE_ROUTE]
    , [CLIENT_CODE]
    , [WAVE_PICKING_ID]
    , [MATERIAL_ID]
    ...
    , [CERTIFICATION_TYPE])
      ...
      INNER JOIN [wms].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL] [DD]
