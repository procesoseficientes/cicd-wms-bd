...
BEGIN
  ...
  SELECT @CLIEN_LICENSE = [VALUE] FROM wms.OP_WMS_PARAMETER WHERE GROUP_ID = 'LICENSING' AND PARAMETER_ID = 'CLIENT_LICENSE'
  SELECT @DATE_EXPIRED = [VALUE] FROM wms.OP_WMS_PARAMETER WHERE GROUP_ID = 'LICENSING' AND PARAMETER_ID = 'DATE_EXPIRED'
  SELECT @TOLERANCIA = [VALUE] FROM wms.OP_WMS_PARAMETER WHERE GROUP_ID = 'VALIDATION_LICENSE' AND PARAMETER_ID = 'VALIDATION_DAYS'
  SELECT @USERS_ONLINE = COUNT(*) FROM wms.OP_WMS_LOGINS WHERE IS_LOGGED = 1
