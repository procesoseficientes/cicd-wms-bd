  ...
  -- ------------------------------------------------------------------------------------
    ...
    -- ------------------------------------------------------------------------------------
			SELECT DISTINCT
				...
				,NULL AS [PHYSICAL_COUNT_HEADER_ID]
			FROM
				[wms].[OP_WMS_VIEW_TASK_PICKING_HEADER] [VT] WITH (NOLOCK)
			...
			INNER JOIN [#CLASS] [C] WITH (NOLOCK) ON [C].[CLASS_ID] = CAST([M].[MATERIAL_CLASS] AS INT)
			INNER JOIN [#LOCATION_TARGET_PICKING] [LTP] WITH (NOLOCK) ON [LTP].[WAVE_PICKING_ID] = [VT].[WAVE_PICKING_ID]
    ...
    -- ------------------------------------------------------------------------------------
			...
			INNER JOIN [wms].[OP_WMS_PHYSICAL_COUNTS_HEADER] [CH] WITH (NOLOCK) ON [T].[TASK_ID] = [CH].[TASK_ID]
			INNER JOIN [wms].[OP_WMS_PHYSICAL_COUNTS_DETAIL] [CD] WITH (NOLOCK) ON [CH].[PHYSICAL_COUNT_HEADER_ID] = [CD].[PHYSICAL_COUNT_HEADER_ID]
			INNER JOIN [#USERS] [U] WITH (NOLOCK) ON  ([U].[login] = [CD].[ASSIGNED_TO])
    ...
    -- ------------------------------------------------------------------------------------
			SELECT DISTINCT
				...
				,NULL AS [PHYSICAL_COUNT_HEADER_ID]
			FROM
				[wms].[OP_WMS_VIEW_TASK_REALLOC_HEADER] [VT] WITH (NOLOCK)
			INNER JOIN @TB_WAREHOUSE [W] ON ([W].[WAREHOUSE_ID] = [VT].[WAREHOUSE_SOURCE])
			INNER JOIN [wms].[OP_WMS_TASK_LIST] [T] WITH (NOLOCK) ON [T].[WAVE_PICKING_ID] = [VT].[WAVE_PICKING_ID]
			INNER JOIN [wms].[OP_WMS_MATERIALS] [M] WITH (NOLOCK) ON [M].[MATERIAL_ID] = [T].[MATERIAL_ID]
			INNER JOIN [#CLASS] [C] WITH (NOLOCK) ON [C].[CLASS_ID] = CAST([M].[MATERIAL_CLASS] AS INT)
			INNER JOIN [#LOCATION_TARGET_REALOC] [LTR] WITH (NOLOCK) ON [LTR].[WAVE_PICKING_ID] = [VT].[WAVE_PICKING_ID]
