...
-- Fecha:				19-Mayo-2021
-- Descripcion:			Se agrega validación de estado de material para la Task list al momento de ingresar 

-- =============================================
CREATE PROCEDURE [wms].[OP_WMS_SP_REGISTER_DISPATCH_BY_REGIME_GENERAL_CDL] (
...
BEGIN
    ...
    ---------------------------------------------------------------------------------
		...
		SELECT TOP 1 @LOGIN_NAME = [D].[LOGIN_NAME]
			FROM [wms].[OP_WMS_FUNC_GETLOGIN_NAME](@pLOGIN_ID) [D];
		PRINT 'CONS1'
		...
		FROM [wms].[OP_WMS_LICENSES] [L] INNER JOIN [wms].[OP_WMS_VIEW_CLIENTS] [C] ON [C].[CLIENT_CODE] = [L].[CLIENT_OWNER]
...
-- ///////////////////////////////////////////////////////////////////////////////////
		...
		FROM [wms].[OP_WMS_INV_X_LICENSE] [IL] LEFT JOIN [wms].[OP_WMS_STATUS_OF_MATERIAL_BY_LICENSE] [SML] ON 
			([IL].[LICENSE_ID] = [SML].[LICENSE_ID] AND [IL].[STATUS_ID] = [SML].[STATUS_ID])
		WHERE
			[IL].[LICENSE_ID] = @pSOURCE_LICENSE AND [IL].[MATERIAL_ID] = @pMATERIAL_ID_LOCAL;
		PRINT 'CONS4'
	---------------------------------------------------------------------------------
    -- VALIDACIÓN DE SI LA TAREA EXISTE
    ---------------------------------------------------------------------------------
		IF NOT EXISTS (SELECT TOP 1 BARCODE_ID FROM wms.OP_WMS_TASK_LIST WHERE 
			WAVE_PICKING_ID = @pWAVE_PICKING_ID AND 
			BARCODE_ID = @pMATERIAL_BARCODE AND 
			MATERIAL_ID = @pMATERIAL_ID AND 
			...
			--			WHERE [ID] = @PROJECT_ID_LICENSE;
			--			PRINT 'CONS6'
			--	END;		

		SELECT TOP 1 @WAREHOUSE_PARENT = ISNULL([WAREHOUSE_PARENT],'BODEGA_DEF')
			FROM [wms].[OP_WMS_SHELF_SPOTS]
				WHERE [LOCATION_SPOT] = @pSOURCE_LOCATION;
		PRINT 'CONS5'

	---------------------------------------------------------------------------------
    -- VALIDACIÓN PARA LAS TAREAS INCOMPLETAS 
    ---------------------------------------------------------------------------------
		IF NOT EXISTS ( SELECT TOP 1 1 FROM [wms].[OP_WMS_TASK_LIST] [TL]
			WHERE [TL].[WAVE_PICKING_ID] = @pWAVE_PICKING_ID AND 
				  [TL].[MATERIAL_ID] = @pMATERIAL_ID_LOCAL AND 
				  [TL].[IS_COMPLETED] = 0 )
		BEGIN
			SELECT
	...
	---------------------------------------------------------------------------------
    -- VALIDACIÓN SI LA CANTIDAD DEL DOCUMENTO ES MAYOR A LA CANTIDAD PENDIENTE
    ---------------------------------------------------------------------------------
		IF @pQUANTITY_UNITS > (SELECT SUM(ISNULL([TL].[QUANTITY_PENDING],0))
			FROM [wms].[OP_WMS_TASK_LIST] [TL]
				WHERE [TL].[WAVE_PICKING_ID] = @pWAVE_PICKING_ID AND
					  [TL].[MATERIAL_ID] = @pMATERIAL_ID_LOCAL)
		BEGIN
			SELECT
    ...
    ---------------------------------------------------------------------------------  
		IF (NOT EXISTS ( SELECT TOP 1 1 FROM [wms].[OP_WMS_TASK_LIST] [TL] WHERE
			[TL].[WAVE_PICKING_ID] = @pWAVE_PICKING_ID AND
		    [TL].[MATERIAL_ID] = @pMATERIAL_ID_LOCAL AND
			[TL].[LICENSE_ID_SOURCE] = @pSOURCE_LICENSE )
				OR EXISTS ( SELECT TOP 1 1 FROM [wms].[OP_WMS_TASK_LIST] [TL] WHERE
					[TL].[WAVE_PICKING_ID] = @pWAVE_PICKING_ID AND
					[TL].[MATERIAL_ID] = @pMATERIAL_ID_LOCAL AND
					[TL].[LICENSE_ID_SOURCE] = @pSOURCE_LICENSE AND 
					([TL].[QUANTITY_PENDING] = 0 OR 
					[TL].[QUANTITY_PENDING] < @pQUANTITY_UNITS) 
				)
			)
		AND @HANDLE_BATCH = 0
      ...
      ---------------------------------------------------------------------------------  
	  PRINT 'pick ' +CAST(@pWAVE_PICKING_ID AS VARCHAR)
	  PRINT 'material ' +CAST(@pMATERIAL_ID_LOCAL AS VARCHAR)
	  PRINT 'license ' +CAST(@pSOURCE_LICENSE AS VARCHAR)
...
print 'hola17'
		...
		PRINT 'WAVE_PICKING_ID  '+ CAST(@pWAVE_PICKING_ID AS VARCHAR)
		PRINT 'MATERIAL_ID  '+ CAST(@pMATERIAL_ID_LOCAL AS VARCHAR)
		PRINT 'LICENSE_ID_SOURCE  '+ CAST(@pSOURCE_LICENSE AS VARCHAR)
		print 'hola23.11111'
		...
		SELECT * FROM [wms].[OP_WMS_TASK_LIST] WHERE [WAVE_PICKING_ID] = 75111
			AND [MATERIAL_ID] = 'ALZA/10105'
			AND [LICENSE_ID_SOURCE] = 646008
