-- =============================================
-- Autor:				Diego Espinoza
-- Fecha de Creacion: 	Octubre 6, 2020
-- Description:			Para ejecutar BulkData en Trigger de SAE y sincronizar en automatico el SKU creado.
--						Se filtrara por SKU.
--Req. By:				Diego E.

CREATE PROCEDURE [wms].[BULK_DATA_SP_IMPORT_SKU_ALZA_DEM]
	@SKU	VARCHAR(20)
AS
BEGIN
    SET NOCOUNT ON;
    ...
    MERGE [wms].[OP_WMS_CLASS] [C]
    USING
    (
        SELECT [FAMILY_CODE],
               [FAMILY_NAME],
               [PRIORITY]
        FROM [wms].[VIEW_SKU_FAMILIES]
    ) AS [F]
    ...
    -- ------------------------------------------------------------------------------------
	MERGE [wms].[OP_WMS_SUB_CLASS] [SC]
    USING
    (
        SELECT [SUB_FAMILY_NAME]
        FROM [wms].[VIEW_SKU_SUB_FAMILIES]
    ) AS [SF]
    ...
    (
        SELECT [V].[CLIENT_OWNER],
			   ...
			   [SC].[SUB_CLASS_ID],
			   [SC].[SUB_CLASS_NAME]
        FROM [wms].[VIEW_SKU_ERP] [V]
            INNER JOIN [wms].[OP_WMS_CLASS] [C] ON [V].[Family]  COLLATE DATABASE_DEFAULT = [C].[CLASS_NAME] COLLATE DATABASE_DEFAULT
			LEFT JOIN [wms].[OP_WMS_SUB_CLASS] [SC] ON [V].[MATERIAL_SUB_CLASS] COLLATE DATABASE_DEFAULT= [SC].[SUB_CLASS_NAME]  COLLATE DATABASE_DEFAULT
    ) AS [V]
