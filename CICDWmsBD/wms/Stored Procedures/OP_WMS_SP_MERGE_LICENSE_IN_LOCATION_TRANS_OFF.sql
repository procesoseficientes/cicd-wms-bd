...
-- Ejemplo de Ejecucion:
				 EXEC [wms].[OP_WMS_SP_MERGE_LICENSE_IN_LOCATION]  
						@LOCATION  = 'PISO1'
				 		,@MATERIAL_ID  = 'me_llega/U00000539'
						,@LOGIN  = 'PABS'

		SELECT [L].[CURRENT_LOCATION], IL.* FROM [wms].[OP_WMS_INV_X_LICENSE] IL INNER JOIN [wms].[OP_WMS_LICENSES] L ON [L].[LICENSE_ID] = [IL].[LICENSE_ID] WHERE [QTY] > 0 AND L.CURRENT_LOCATION = 'PISO1' ORDER BY MATERIAL_ID

		SELECT * FROM [wms].OP_WMS_TRANS where license_id IN (14418,14419)

		select * from [wms].OP_WMS_INV_X_LICENSE WHERE MATERIAL_ID = 'me_llega/U00000539'
		

*/
-- =============================================
CREATE PROCEDURE [wms].[OP_WMS_SP_MERGE_LICENSE_IN_LOCATION_TRANS_OFF]
(
    @LOCATION VARCHAR(25),
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            FROM [wms].[OP_WMS_ACUERDOS_X_CLIENTE] [AC]
                LEFT JOIN [wms].[OP_WMS_TARIFICADOR_HEADER] [T]
