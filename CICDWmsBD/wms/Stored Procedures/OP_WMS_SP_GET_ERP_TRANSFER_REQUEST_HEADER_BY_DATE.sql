...
-- Ejemplo de Ejecucion:
				EXEC [wms].[OP_WMS_SP_GET_ERP_TRANSFER_REQUEST_HEADER_BY_DATE]
					@WAREHOUSE_ID = 'BODEGA_C002'
					,@START_DATETIME = '2016-08-23 00:00:00.000'
					,@END_DATETIME = '2018-08-23 00:00:00.000'
*/
-- =============================================
CREATE PROCEDURE [wms].[OP_WMS_SP_GET_ERP_TRANSFER_REQUEST_HEADER_BY_DATE] (
      ...
      -- ------------------------------------------------------------------------------------
			SELECT
				...
				+ '.[ERP_VW_TRANSFER_REQUEST_HEADER] [TRH]
					INNER JOIN [wms].[OP_WMS_WAREHOUSES] [W1] ON ([W1].[ERP_WAREHOUSE] COLLATE DATABASE_DEFAULT = [TRH].[FROM_WAREHOUSE_CODE] COLLATE DATABASE_DEFAULT)
					INNER JOIN [wms].[OP_WMS_WAREHOUSES] [W2] ON ([W2].[ERP_WAREHOUSE] COLLATE DATABASE_DEFAULT = [TRH].[TO_WAREHOUSE_CODE] COLLATE DATABASE_DEFAULT)
          INNER JOIN [wms].[OP_WMS_COMPANY] [C] ON [TRH].[SOURCE] COLLATE DATABASE_DEFAULT  = [C].[CLIENT_CODE] COLLATE DATABASE_DEFAULT AND [C].[EXTERNAL_SOURCE_ID] = '
				+ CAST(@EXTERNAL_SOURCE_ID AS VARCHAR) + '
