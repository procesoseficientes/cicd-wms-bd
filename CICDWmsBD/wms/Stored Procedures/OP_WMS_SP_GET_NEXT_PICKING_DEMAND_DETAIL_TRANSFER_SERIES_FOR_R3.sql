...
-- Ejemplo de Ejecucion:
			...
			--
			EXEC [wms].[OP_WMS_SP_GET_NEXT_PICKING_DEMAND_DETAIL_TRANSFER_SERIES_FOR_R3] 
				@PICKING_DEMAND_HEADER_ID = 26
*/
-- =============================================
CREATE PROCEDURE [wms].[OP_WMS_SP_GET_NEXT_PICKING_DEMAND_DETAIL_TRANSFER_SERIES_FOR_R3]
...
BEGIN
    ...
    SELECT CASE
               WHEN [MS].[CORRELATIVE] IS NULL THEN
                   [PDD].[QTY]
               ELSE
                   1
           END [MENGE],
           ...
           CASE
               WHEN [MS].[CORRELATIVE] IS NULL THEN
                   ''
               WHEN [MS].[SERIAL_CORRELATIVE] IS NULL THEN
                   [MS].[SERIAL]
               ELSE
                   ISNULL([MS].[SERIAL_PREFIX], '') + CAST([MS].[SERIAL_CORRELATIVE] AS VARCHAR(50))
           END [SERIE],
           [PDH].[WAVE_PICKING_ID],
           [M].[MATERIAL_ID],
    ...
    FROM [wms].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL] [PDD]
        ...
        INNER JOIN [SWIFT_R3_INTER].[dbo].[RFC_PURCHASES] [SO]
            ON [PDH].[DOC_ENTRY] = [SO].[EBELN]
               AND [SO].[EBELP] = [PDD].[LINE_NUM]
        INNER JOIN [wms].[OP_WMS_MATERIALS] [M]
            ON [M].[MATERIAL_ID] = [PDD].[MATERIAL_ID]
        INNER JOIN [wms].[OP_WMS_CONFIGURATIONS] [CM]
            ON [CM].[PARAM_NAME] = [M].[BASE_MEASUREMENT_UNIT]
               AND [CM].[PARAM_GROUP] = 'TIPOS_EMPAQUE'
        INNER JOIN [wms].[OP_WMS_COMPANY] [C]
            ON [C].[EXTERNAL_SOURCE_ID] = [PDH].[EXTERNAL_SOURCE_ID]
               AND [C].[CLIENT_CODE] = [PDH].[CLIENT_CODE]
        INNER JOIN [wms].[OP_WMS_TASK_LIST] [T]
            ON [T].[WAVE_PICKING_ID] = [PDH].[WAVE_PICKING_ID]
        LEFT JOIN [wms].[OP_WMS_MATERIAL_X_SERIAL_NUMBER] [MS]
            ON [MS].[PICKING_HEADER_ID] = [PDH].[PICKING_DEMAND_HEADER_ID]
    ...
    ORDER BY [PDD].[PICKING_DEMAND_DETAIL_ID] ASC;
    ...
    WHILE EXISTS
          (
              SELECT TOP 1
                     1
              FROM [#PICKING_SERIES]
              WHERE [SERIAL_NUMBER_REQUESTS] = 1
                    AND [SERIE] = ''
          )
    BEGIN
        ...
        PRINT @MATERIAL_ID + ' LINEA:' + CAST(@LINE_NUM AS VARCHAR) + ' ' + CAST(@QTY AS VARCHAR);
        WHILE (@QTY > 0)
        BEGIN
            SELECT TOP 1
                   @SERIAL_NUMBER_ID = [CORRELATIVE]
            FROM [wms].[OP_WMS_MATERIAL_X_SERIAL_NUMBER] [MS]
            WHERE [MS].[PICKING_HEADER_ID] IS NULL
                  AND [MS].[WAVE_PICKING_ID] = @WAVE_PICKING_ID
                  AND [MS].[MATERIAL_ID] = @MATERIAL_ID;

            UPDATE [wms].[OP_WMS_MATERIAL_X_SERIAL_NUMBER]
            SET [PICKING_HEADER_ID] = @PICKING_DEMAND_HEADER_ID,
                [PICKING_LINE_NUM] = @LINE_NUM
            WHERE @SERIAL_NUMBER_ID = [CORRELATIVE];
            ...
            SELECT TOP 1
                   ...
                   [R].[MATERIAL_ID],
                   [R].[ROW_NUMBER]
            FROM [#PICKING_SERIES] [R]
                INNER JOIN [wms].[OP_WMS_MATERIAL_X_SERIAL_NUMBER] [MS]
                    ON [MS].[CORRELATIVE] = @SERIAL_NUMBER_ID
            WHERE [EBELP] = @LINE_NUM;


            SELECT @QTY = @QTY - 1;
        END;
        DELETE [#PICKING_SERIES]
        WHERE [EBELP] = @LINE_NUM
              AND [SERIAL_NUMBER_REQUESTS] = 1;
    END;

    SELECT [ROW_NUMBER] [EBELP],
           [SERIE]
    FROM [#PICKING_SERIES]
    WHERE ISNULL([SERIE], '') <> ''
    ORDER BY [EBELP] ASC,
