...
BEGIN
  ...
  ---------------------------------------------------------------------------------  
	...
	BEGIN
			...
			END
				...
				SELECT MASTER_PACK_CODE, COMPONENT_MATERIAL, QTY INTO #KIT_DETAIL FROM WMS.OP_WMS_COMPONENTS_BY_MASTER_PACK WHERE MASTER_PACK_CODE = @MATERIAL_ID
					...
					BEGIN
						...
						IF (@MASTERPACK_COMPONENT_TOTAL_QTY > 0 AND (@QTY_TO_REPLENISH > ( @MASTERPACK_COMPONENT_TOTAL_QTY/@MASTERPACK_COMPONENT_QTY)))
						BEGIN
							SELECT @QTY_TO_REPLENISH = (@MASTERPACK_COMPONENT_TOTAL_QTY/@MASTERPACK_COMPONENT_QTY)
						END
		...
		SET @QTY_TO_REPLENISH = FLOOR(@QTY_TO_REPLENISH)
