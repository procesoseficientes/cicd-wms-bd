...
BEGIN
	...
	END
	ELSE IF (@INSUFFICIENT_FOOR_QTY = 1 AND ISNULL(@FOR_FORKLIFT, 0) = 1 AND ISNULL((SELECT TOP 1 1 FROM @TAKEN_LICENSES WHERE LICENSE_ID = @LICENSE_ID), 0) = 0)
	BEGIN
		IF(@QTY >= @QTY_REQUIRED)
			BEGIN
				INSERT INTO @TAKEN_LICENSES(LICENSE_ID) VALUES(@LICENSE_ID)
	...
	BEGIN 
		SELECT @POSITION = 0, @INSUFFICIENT_FOOR_QTY = 1
