...
*/
-- =============================================
CREATE PROCEDURE [dbo].[SAE_CREATE_INVENTORY_INCOME_BY_PURCHASE_ORDERgh]
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT [D].[ERP_RECEPTION_DOCUMENT_DETAIL_ID],
               ...
               isnull(mc.MATERIAL_ID,d.[MATERIAL_ID]) MATERIAL_ID ,
               [D].[QTY]*isnull(c.QTY,1) qty,
               [D].[LINE_NUM],
               [D].[ERP_OBJECT_TYPE],
               [D].[WAREHOUSE_CODE],
               [D].[CURRENCY],
               ...
               [D].[TAX_CODE],
               [D].[VAT_PERCENT],
               [D].[PRICE]/isnull(c.QTY,1) price,
               [D].[DISCOUNT],
               [D].[COST_CENTER],
               [D].[QTY_ASSIGNED]*isnull(c.QTY,1) [QTY_ASSIGNED],
               [D].[UNIT],
               [D].[UNIT_DESCRIPTION],
               [D].[QTY_CONFIRMED]*isnull(c.QTY,1) [QTY_CONFIRMED],
               [D].[IS_CONFIRMED],
               0 [ENVIADO],
               isnull(mc.[ITEM_CODE_ERP],m.[ITEM_CODE_ERP]) [ITEM_CODE_ERP],
               ...
               [CD].[NUM_PAR],
               [CD].[CVE_ART],
               [CD].[CANT]*isnull(c.QTY,1)	CANT,
               [CD].[PXR],
               [CD].[PREC]/isnull(c.QTY,1) PREC,
               [CD].[COST]/isnull(c.QTY,1) [COST],
               [CD].[IMPU1]/isnull(c.QTY,1) [IMPU1],
               [CD].[IMPU2]/isnull(c.QTY,1)[IMPU2],
               [CD].[IMPU3]/isnull(c.QTY,1)[IMPU3],
               [CD].[IMPU4]/isnull(c.QTY,1)[IMPU4],
               [CD].[IMP1APLA],
               [CD].[IMP2APLA],
               [CD].[IMP3APLA],
               [CD].[IMP4APLA],
               ...
               [CD].[REG_SERIE],
               [CD].[FACTCONV],
               [CD].[COST_DEV]/isnull(c.QTY,1)[COST_DEV],
               ISNULL(D.WAREHOUSE_CODE,[CD].[NUM_ALM]) [NUM_ALM],
               [CD].[MINDIRECTO],
               [CD].[NUM_MOV],
               [CD].[TOT_PARTIDA]/isnull(c.QTY,1)[TOT_PARTIDA],
               [CD].[MAN_IEPS],
               [CD].[APL_MAN_IMP],
               [CD].[CUOTA_IEPS],
               [CD].[APL_MAN_IEPS],
               ...
               CAST(0 AS INT) [NUMERO_MOVIMIENTO],
			   	([CD].COST-[CD].COST*[CD].[DESCU]/100)/isnull(c.QTY,1) UNITARIO_CON_DESCUENTO,
      ...
      INTO [#DETALLE]
       FROM [OP_WMS_ALZA].[wms].[OP_WMS_ERP_RECEPTION_DOCUMENT_DETAIL] [D]
			...
			left join op_wms_alza.wms.OP_WMS_COMPONENTS_BY_MASTER_PACK c 
				on c.MASTER_PACK_CODE=d.MATERIAL_ID 
					and m.IS_MASTER_PACK=1
			left join op_wms_alza.wms.OP_WMS_MATERIALS MC
				on mc.MATERIAL_ID = c.COMPONENT_MATERIAL
        ...
        BEGIN
            ...
            SET [ENVIADO] = 1,
				...
				COSTO_ANTERIOR=@COSTO_PROMEDIO_ANTERIOR
            WHERE [LINE_NUM] = @LINE_NUM_DETAIL;
        END;
