...
-- Description:			Se agrega manejo de cantidades en task list para enviar a SAE solo las cantidades verdaderas operadas
/*
-- Ejemplo de Ejecucion:
				EXEC [dbo].[SAE_CREATE_REMISION_BY_SALE_ORDER] @NEXT_PICKING_DEMAND_HEADER = 63750 -- numeric
				rollback
				
				
*/
-- =============================================
CREATE PROCEDURE [dbo].[SAE_CREATE_REMISION_BY_SALE_ORDER_20042022]
...
BEGIN
    ...
    DECLARE @TABLA_DOCUMENTO_32 INT = 32,
			...
			@DEMAND_TYPE VARCHAR(255) = 'SALES_ORDER';
...
SELECT [D].[PICKING_DEMAND_DETAIL_ID],
        ...
        -- ------------------------------------------------------------------------------------
		SELECT TOP 1 @ALMACEN=[NUM_ALM] FROM [#DETALLE]
	   ...
	   @pRESULT VARCHAR(200)=''; 
		...
		BEGIN
			...
			print @pRESULT
			SELECT -1 AS [Resultado],
               'Proceso fallido: ' + @pRESULT [Mensaje],
               0 [Codigo],
               '0' [DbData];
			RETURN;
		END;
		...
		--validamos existencias desde el inicio:
       ...
       -- PRINT 'Comentario ' + CAST(@COMENTARIO AS VARCHAR(250));
        ...
        BEGIN
            ...
            SELECT TOP 1
                   ...
                   [D].[QTY],
                   0,
                   [D].[PREC] - (ISNULL([D].DESC1,0)*[D].[PREC]/100),
		...
		--  se modifica para calcule el descuento antes de aplicar el impuesto
        -- ------------------------------------------------------------------------------------
        SELECT @TOTAL_DOCUMENTO = SUM([D].[QTY] * [D].[PRICE]),
               @TOTAL_IMPUESTO_04 = SUM([D].[QTY] * [D].UNITARIO_CON_DESCUENTO * [D].[IMPU4] / 100),
               @TOTAL_IMPUESTO_03 = SUM([D].[QTY] * [D].UNITARIO_CON_DESCUENTO * [D].[IMPU3] / 100),
               @TOTAL_IMPUESTO_02 = SUM([D].[QTY] * [D].UNITARIO_CON_DESCUENTO * [D].[IMPU2] / 100),
               @TOTAL_IMPUESTO_01 = SUM([D].[QTY] * [D].UNITARIO_CON_DESCUENTO * [D].[IMPU1] / 100),
               @TOTAL_IMPORTE
                   = SUM([D].[QTY] * [D].UNITARIO_CON_DESCUENTO) + SUM([D].[QTY] * [D].UNITARIO_CON_DESCUENTO * [D].[IMPU4] / 100)
                     + SUM([D].[QTY] * [D].UNITARIO_CON_DESCUENTO * [D].[IMPU3] / 100)
                     + SUM([D].[QTY] * [D].UNITARIO_CON_DESCUENTO * [D].[IMPU2] / 100)
                     + SUM([D].[QTY] * [D].UNITARIO_CON_DESCUENTO * [D].[IMPU1] / 100),
				@TOTAL_DESCUENTO = SUM([D].[QTY] * [D].[PREC]*ISNULL([D].[DESC1],0)/100)
        FROM [#DETALLE] [D];
        ...
        -- ------------------------------------------------------------------------------------
        INSERT INTO [SAE70EMPRESA01].[dbo].[FACTR01]
        (
            [TIP_DOC],
            [CVE_DOC],
            [CVE_CLPV],
        ...
        SELECT @TIPO_DOCUMENTO_FOLIO [TIP_DOC],
               ...
               @TOTAL_DESCUENTO,--[H].[DES_TOT], --valor total de descuento
               [H].[DES_FIN],
               [H].[COM_TOT] [COM_TOT],
               ...
               @TOTAL_IMPORTE [IMPORTE], --impuesto - descuento
               'PPD' [METODODEPAGO],
        ...
        FROM [#ENCABEZADO] [H];
		PRINT 'termina 2';
        INSERT INTO [SAE70EMPRESA01].[dbo].[FACTR_CLIB01]
        (
            [CLAVE_DOC],
            [CAMPLIB1],
            [CAMPLIB2],
        ...
        BEGIN
            ...
            INSERT INTO [SAE70EMPRESA01].[dbo].[PAR_FACTR01]
            (
                [CVE_DOC],
                [NUM_PAR],
                [CVE_ART],
            ...
            SELECT @DOCUMENTO_ERP_FORMATEADO [CVE_DOC],
                   ...
                   [D].[QTY] [PXS],
                   [D].[PREC] [PREC],
                   (SELECT COSTO_PROM FROM [SAE70EMPRESA01].[dbo].[INVE01] WHERE [CVE_ART] = @ERP_MATERIAL_CODE), -- [D].[COST] [COST],
                   [D].[IMPU1] [IMPU1],
                   [D].[IMPU2] [IMPU2],
                   [D].[IMPU3] [IMPU3],
                   [D].[IMPU4] [IMPU4],
                   ...
                   [D].[IMP3APLA] [IMP3APLA],
                   [D].[IMP4APLA] [IMP4APLA],
                   CAST(([D].[QTY] * [D].[IMPU1]*D.UNITARIO_CON_DESCUENTO) / 100 AS FLOAT) [TOTIMP1],
                   CAST(([D].[QTY] * [D].[IMPU2]*D.UNITARIO_CON_DESCUENTO) / 100 AS FLOAT) [TOTIMP2],
                   CAST(([D].[QTY] * [D].[IMPU3]*D.UNITARIO_CON_DESCUENTO) / 100 AS FLOAT) [TOTIMP3],
                   CAST(([D].[QTY] * [D].[IMPU4]*D.UNITARIO_CON_DESCUENTO) / 100 AS FLOAT) [TOTIMP4],
                   ISNULL([D].[DESC1],0) [DESC1],
                   [D].[DESC2] [DESC2],
                   [D].[DESC3] [DESC3],
                   [D].[COMI] [COMI],
                   [D].[APAR] [APAR],
                   ...
                   CASE WHEN [D].[IMPU4] = 0 THEN 2 ELSE 1 END  [CVE_ESQ],
                   [D].[UUID] [UUID],
                   @FECHA_SYNC [VERSION_SINC]
            FROM [#DETALLE] [D]
            WHERE [LINE_NUM] = @LINE_NUM_DETAIL;
        ...
        SET [TIP_DOC_E] = @TIPO_DOCUMENTO_FOLIO,
            ...
            [ENLAZADO] = (CASE
                              ...
                              (
                                  SELECT SUM([P].[PXS])
                                  FROM [SAE70EMPRESA01].[dbo].[PAR_FACTP01] [P]
