...
-- Fecha de Creación:   05-May-21 @ Sprint 11 DES-370
-- Descripción:			Se modifica la forma en calcular el impuesto para que no se aplique a el descuento
--						Se corrije problema con el numero siguiente
--						Se cmbia los valores de costo para que se aplique descuento.
/*
...
*/
-- =============================================
CREATE PROCEDURE [dbo].[SAE_CREATE_INVENTORY_INCOME_BY_CREDIT_MEMO19062021]
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            SELECT TOP 1
                   ...
                   0,
                   [PREC] - ([PREC]*DESC1/100),
                   @COSTO_ARTICULO_DOCUMENTO - (@COSTO_ARTICULO_DOCUMENTO*DESC1/100 ) [COSTO],
                   0 [REG_SERIE],
        ...
        SELECT @TOTAL_COMPRA = SUM([D].[QTY_CONFIRMED] * [D].[PREC]),
               @TOTAL_IMPUESTO_04
                   = SUM([D].[QTY_CONFIRMED] *UNITARIO_CON_DESCUENTO
                         * ([D].[IMPU4] / 100)
                        ),
               @TOTAL_IMPUESTO_03
                   = SUM([D].[QTY_CONFIRMED] * UNITARIO_CON_DESCUENTO
        ...
        BEGIN
            ...
            SELECT @DOCUMENTO_ERP_FORMATEADO [CVE_DOC],
                   ...
                   [D].[IMP3APLA],
                   [D].[IMP4APLA],
                   CAST(([D].[QTY_CONFIRMED] * UNITARIO_CON_DESCUENTO * [D].[IMPU1]) / 100 AS FLOAT) [TOTIMP1],
                   CAST(([D].[QTY_CONFIRMED] * UNITARIO_CON_DESCUENTO * [D].[IMPU2]) / 100 AS FLOAT) [TOTIMP2],
                   CAST(([D].[QTY_CONFIRMED] * UNITARIO_CON_DESCUENTO * [D].[IMPU3]) / 100 AS FLOAT) [TOTIMP3],
                   CAST(([D].[QTY_CONFIRMED] * UNITARIO_CON_DESCUENTO * [D].[IMPU4]) / 100 AS FLOAT) [TOTIMP4],
                   [D].[DESC1],
                   [D].[DESC2],
                   [D].[DESC3],
                   [D].[COMI],
                   ...
                   0 [CVE_OBS],
                   [REG_SERIE],
                   '',--no usan esta funcionalidad indica Gerson
