...
-- Fecha Modificación:		28 de marzo de 2022
-- Descripción:				Se obtiene el folio de la tabla FOLIOSWMS para las devoluciones
/*
-- Ejemplo de Ejecucion:
				EXEC [dbo].[SAE_CREATE_INVENTORY_INCOME_BY_CREDIT_MEMO] @RECEPTION_DOCUMENT_HEADER = 725 -- numeric
				ROLLBACK
*/
-- =============================================
CREATE PROCEDURE [dbo].[SAE_CREATE_INVENTORY_INCOME_BY_CREDIT_MEMO]
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT [D].[ERP_RECEPTION_DOCUMENT_DETAIL_ID],
			   ...
			   [CD].[PREC]-[CD].[PREC]*[CD].[DESC1]/100 UNITARIO_CON_DESCUENTO
        INTO [#DETALLE]
        FROM [OP_WMS_ALZA].[wms].[OP_WMS_ERP_RECEPTION_DOCUMENT_DETAIL] [D] WITH (NOLOCK)
            INNER JOIN [OP_WMS_ALZA].[wms].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [H] WITH (NOLOCK)
            ...
            INNER JOIN [SAE70EMPRESA01].[dbo].[FACTF01] [CH] WITH (NOLOCK)
                ON LTRIM(RTRIM([CH].[CVE_DOC])) COLLATE DATABASE_DEFAULT = [H].[DOC_ID] COLLATE DATABASE_DEFAULT
            INNER JOIN [SAE70EMPRESA01].[dbo].[PAR_FACTF01] [CD] WITH (NOLOCK)
        ...
        SELECT [H].[DOC_ID],
               ...
               [CL].[CAMPLIB21]
        INTO [#ENCABEZADO]
        FROM [OP_WMS_ALZA].[wms].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [H] WITH (NOLOCK)
            INNER JOIN [SAE70EMPRESA01].[dbo].[FACTF01] [CH] WITH (NOLOCK)
                ON RTRIM(LTRIM([CH].[CVE_DOC])) COLLATE DATABASE_DEFAULT = [H].[DOC_ID] COLLATE DATABASE_DEFAULT
            INNER JOIN [SAE70EMPRESA01].[dbo].[FACTF_CLIB01] [CL] WITH (NOLOCK)
        ...
        FROM [SAE70EMPRESA01].[dbo].[CLIE01] [P]
		...
		-- ------------------------------------------------------------------------------------
		-- obtiene el folio correspondiente a la devolción
		-- ------------------------------------------------------------------------------------

		DECLARE 
			@CLASIFICACIÓN VARCHAR(10),
			@BODEGA_FOLIO VARCHAR(15)

		SELECT 
			@CLASIFICACIÓN = FW.Clasificacion,
			@BODEGA_FOLIO = FW.Bodega
		FROM SAE70EMPRESA01.dbo.FOLIOSWMS FW
		INNER JOIN [SAE70EMPRESA01].[dbo].[FACTF01] FF
			ON FW.Folio = FF.SERIE
		INNER JOIN OP_WMS_ALZA.WMS.OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER H
			ON LTRIM(RTRIM([FF].[CVE_DOC])) COLLATE DATABASE_DEFAULT = [H].[DOC_ID] COLLATE DATABASE_DEFAULT 
			AND H.ERP_RECEPTION_DOCUMENT_HEADER_ID = @RECEPTION_DOCUMENT_HEADER

		SELECT 
			@SERIE_FOLIO = FW.Folio
		FROM SAE70EMPRESA01.dbo.FOLIOSWMS FW
		WHERE Clasificacion = @CLASIFICACIÓN AND Bodega = @BODEGA_FOLIO AND Tipo = 'D'
        ...
        SELECT @TOTAL_COMPRA = SUM([D].[QTY_CONFIRMED] * [D].[PREC]),
               ...
               @TOTAL_IMPORTE
                   = SUM([D].[QTY_CONFIRMED] * [D].UNITARIO_CON_DESCUENTO)
                     + SUM([D].[QTY_CONFIRMED] * [D].UNITARIO_CON_DESCUENTO
                           * [D].[IMPU4] / 100
                          )
                     + SUM([D].[QTY_CONFIRMED] * [D].UNITARIO_CON_DESCUENTO
        ...
        INSERT INTO [SAE70EMPRESA01].[dbo].[CUEN_DET01]
        (
            [CVE_CLIE],
            [REFER],
            [NUM_CARGO],
        ...
        INSERT INTO [SAE70EMPRESA01].[dbo].[FACTD01]
        (
            [TIP_DOC],
            [CVE_DOC],
            [CVE_CLPV],
        ...
        INSERT INTO [SAE70EMPRESA01].[dbo].[FACTD_CLIB01]
        (
            [CLAVE_DOC],
            [CAMPLIB1],
            [CAMPLIB2],
        ...
        BEGIN
            ...
            UPDATE [SAE70EMPRESA01].[dbo].[PAR_FACTF01]
            SET [PXS] = (CASE
            ...
            INSERT INTO [SAE70EMPRESA01].[dbo].[PAR_FACTD01]
            (
                [CVE_DOC],
                [NUM_PAR],
                [CVE_ART],
            ...
            SELECT @DOCUMENTO_ERP_FORMATEADO [CVE_DOC],
                   ...
                   [D].[IMP3APLA],
                   [D].[IMP4APLA],
                   CAST(([D].[QTY_CONFIRMED] * [D].UNITARIO_CON_DESCUENTO * [D].[IMPU1]) / 100 AS FLOAT) [TOTIMP1],
                   CAST(([D].[QTY_CONFIRMED] * [D].UNITARIO_CON_DESCUENTO * [D].[IMPU2]) / 100 AS FLOAT) [TOTIMP2],
                   CAST(([D].[QTY_CONFIRMED] * [D].UNITARIO_CON_DESCUENTO * [D].[IMPU3]) / 100 AS FLOAT) [TOTIMP3],
                   CAST(([D].[QTY_CONFIRMED] * [D].UNITARIO_CON_DESCUENTO * [D].[IMPU4]) / 100 AS FLOAT) [TOTIMP4],
                   [D].[DESC1],
                   [D].[DESC2],
                   [D].[DESC3],
                   [D].[COMI],
            ...
            SET [TIP_DOC_E] = @TIPO_DOCUMENTO_FOLIO,
                ...
                [ENLAZADO] = (CASE
                                  ...
                                  (
                                      SELECT SUM([P].[PXS])
                                      FROM [SAE70EMPRESA01].[dbo].[PAR_FACTF01] [P]
            ...
            INSERT INTO [SAE70EMPRESA01].[dbo].[PAR_FACTD_CLIB01]
            (
                [CLAVE_DOC],
                [NUM_PART]
            )
		...
		set SALDO = SALDO - @TOTAL_COMPRA,
			VENTAS = VENTAS- @TOTAL_COMPRA
		WHERE CLAVE in (select top 1 H.CVE_CLPV from #ENCABEZADO H)
