    ...
    BEGIN TRY
       ...
       INTO [#DETALLE]
        FROM [OP_WMS_ALZA].[wms].[OP_WMS_ERP_RECEPTION_DOCUMENT_DETAIL] [D] WITH (NOLOCK)
            ...
            LEFT JOIN [SAE70EMPRESA01].[dbo].[COMPO01] [CH] WITH (NOLOCK)
                ON LTRIM(RTRIM([CH].[CVE_DOC])) COLLATE DATABASE_DEFAULT = [H].[DOC_ID] COLLATE DATABASE_DEFAULT
            LEFT JOIN [SAE70EMPRESA01].[dbo].[PAR_COMPO01] [CD] WITH (NOLOCK)
        ...
        FROM [OP_WMS_ALZA].[wms].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [H] WITH (NOLOCK)
			...
			INNER JOIN [OP_WMS_ALZA].[wms].[OP_WMS_MANIFEST_HEADER] [MH] WITH (NOLOCK) ON (MH.MANIFEST_HEADER_ID =H.DOC_NUM) 
			INNER JOIN [OP_WMS_ALZA].[wms].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PDH] WITH (NOLOCK) ON (PDH.DOC_NUM = MH.TRANSFER_REQUEST_ID)
