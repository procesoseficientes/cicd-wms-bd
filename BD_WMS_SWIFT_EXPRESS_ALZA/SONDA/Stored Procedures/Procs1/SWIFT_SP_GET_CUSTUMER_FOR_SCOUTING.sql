...
-- Ejemplo de Ejecucion:
        exec [SONDA].[SWIFT_SP_GET_CUSTUMER_FOR_SCOUTING] @CODE_ROUTE = '44'
*/
-- =============================================
...
BEGIN
	...
	ISNULL([CODE_PRICE_LIST], [SONDA].[SWIFT_FN_GET_PARAMETER] ('ERP_HARDCODE_VALUES','PRICE_LIST'))
  FROM [SONDA].[USERS]
  WHERE [SELLER_ROUTE] = 46*/
    ...
    FROM [SONDA].[SWIFT_VIEW_ALL_COSTUMER] [C]
        INNER JOIN [SONDA].[USERS] [U]
            ON ([C].[SELLER_DEFAULT_CODE] = [U].[RELATED_SELLER] COLLATE DATABASE_DEFAULT)
        LEFT JOIN [#DISCOUNT_LIST] [DLC]
            ON (
                   [DLC].[CODE_CUSTOMER] = [C].[CODE_CUSTOMER] COLLATE DATABASE_DEFAULT
                   AND [DLC].[CODE_CUSTOMER] > ''
               )
        LEFT JOIN [#BONUS_LIST] [BLC]
            ON (
                   [BLC].[CODE_CUSTOMER] = [C].[CODE_CUSTOMER] COLLATE DATABASE_DEFAULT
                   AND [BLC].[CODE_CUSTOMER] > ''
               )
        LEFT JOIN [#SKU_SALES_BY_MULTIPLE_LIST] [SMC]
            ON (
                   [SMC].[CODE_CUSTOMER] = [C].[CODE_CUSTOMER] COLLATE DATABASE_DEFAULT
                   AND [SMC].[CODE_CUSTOMER] > ''
               )
        LEFT JOIN [SONDA].[SWIFT_PRICE_LIST_BY_CUSTOMER_FOR_ROUTE] [PLC]
            ON (
                   [PLC].[CODE_CUSTOMER] = [C].[CODE_CUSTOMER] COLLATE DATABASE_DEFAULT
                   AND [PLC].[CODE_CUSTOMER] > ''
               )
        LEFT JOIN [#SPECIAL_PRICE_LIST] AS [SPL]
            ON (
                   [SPL].[CODE_CUSTOMER] = [C].[CODE_CUSTOMER] COLLATE DATABASE_DEFAULT
                   AND [SPL].[CODE_CUSTOMER] > ''
               )
        LEFT JOIN [#CHANNEL] AS [CH]
            ON ([CH].[CODE_CUSTOMER] = [C].[CODE_CUSTOMER] COLLATE DATABASE_DEFAULT) 
    ...
    FROM [SONDA].[SWIFT_VIEW_ALL_COSTUMER] [VAC]
        INNER JOIN [SONDA].[SONDA_ROUTE_PLAN] [RP]
            ON ([VAC].[CODE_CUSTOMER] = [RP].[RELATED_CLIENT_CODE] COLLATE DATABASE_DEFAULT)
        LEFT JOIN [#DISCOUNT_LIST] [DLC]
            ON (
        ...
        LEFT JOIN [SONDA].[SWIFT_PRICE_LIST_BY_CUSTOMER_FOR_ROUTE] [PLC]
            ON (
                   [PLC].[CODE_CUSTOMER] = [VAC].[CODE_CUSTOMER] COLLATE DATABASE_DEFAULT
                   AND [PLC].[CODE_CUSTOMER] > ''
               )
        LEFT JOIN [#SPECIAL_PRICE_LIST] AS [SPL]
            ON (
    ...
    FROM [#CUSTOMER] [C]
        INNER JOIN [#RESULT] [R]
            ON ([R].[CODE_CUSTOMER] = [C].[CODE_CUSTOMER] COLLATE DATABASE_DEFAULT);
