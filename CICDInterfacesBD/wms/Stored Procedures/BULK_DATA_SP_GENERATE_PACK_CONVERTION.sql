...
-- Ejemplo de Ejecucion:
				EXEC [wms].[BULK_DATA_SP_GENERATE_PACK_CONVERTION]
*/
-- =============================================
CREATE PROCEDURE [wms].[BULK_DATA_SP_GENERATE_PACK_CONVERTION]
AS
BEGIN
  SET NOCOUNT ON;
  ...
  -- ------------------------------------------------------------------------------------
  MERGE [SWIFT_EXPRESS].[wms].[SONDA_PACK_UNIT] AS [SPU]
  USING (SELECT
  ...
  -- ------------------------------------------------------------------------------------
  MERGE [SWIFT_EXPRESS].[wms].[SONDA_PACK_CONVERSION] AS [TRG]
  USING (SELECT
      [epc].[CODE_SKU]
     ...
     ,1 [ORDER]
    FROM [SWIFT_EXPRESS].[wms].[SWIFT_VIEW_ALL_SKU] [epc]) AS [SRC]
