...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [wms].[BULK_DATA_SP_IMPORT_ROUTES]
*/
-- =============================================
CREATE PROCEDURE [wms].[BULK_DATA_SP_IMPORT_ROUTES]
AS
BEGIN
  SET NOCOUNT ON;
  --
  MERGE SWIFT_EXPRESS.[wms].[SWIFT_ROUTES] AS TRG
  USING (SELECT
     [EVR].[CODE_ROUTE]
	 ,[EVR].[NAME_ROUTE]
	 ...
	 ,[EVR].[CODE_COUNTRY]
	 ,[EVR].[NAME_COUNTRY]
    FROM [SWIFT_INTERFACES_ONLINE].[wms].[ERP_VIEW_ROUTE] EVR ) AS SRC
