...
-- Ejemplo de Ejecucion:
			EXEC [wms].[OP_WMS_SP_SENT_SAE_TOP5_RECEPTION_DOCUMENT]
				@OWNER = 'ALZA'
*/
-- =============================================
CREATE PROCEDURE [wms].[OP_WMS_SP_SENT_SAE_TOP5_RECEPTION_DOCUMENT]
(@OWNER VARCHAR(50))
AS
BEGIN
    SET NOCOUNT ON;
   ...
   FROM [OP_WMS_ALZA].[wms].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [RDH]
    WHERE [RDH].[ERP_RECEPTION_DOCUMENT_HEADER_ID] > 0
          ...
          AND ISNULL([RDH].[SOURCE],'') <> 'RECEPCION_GENERAL'
		  AND RDH.TYPE <> 'RECEPCION_TRASLADO'; --AND 1 = 2;
