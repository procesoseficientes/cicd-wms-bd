...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [wms].[BULK_DATA_SP_IMPORT_PRICE_LIST_BY_SKU]
				--
				SELECT * FROM [SWIFT_EXPRESS].[wms].[SWIFT_PRICE_LIST_BY_SKU]
*/
-- =============================================
CREATE PROCEDURE [wms].[BULK_DATA_SP_IMPORT_PRICE_LIST_BY_SKU]
AS
BEGIN
	SET NOCOUNT ON;
	--
	MERGE [SWIFT_EXPRESS].[wms].[SWIFT_PRICE_LIST_BY_SKU]   [TGR] 
	USING (SELECT * FROM  [SWIFT_INTERFACES_ONLINE].[wms].[ERP_PRICE_LIST_BY_SKU] WHERE [CODE_PRICE_LIST] IS NOT NULL) [SRC] 
	ON  
