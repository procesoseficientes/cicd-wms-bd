...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [wms].[BULK_DATA_SP_IMPORT_PRICE_LIST_BY_SKU_PACK_SCALE]
				--
				SELECT * FROM [SWIFT_EXPRESS].[wms].[SWIFT_PRICE_LIST_BY_SKU_PACK_SCALE]
*/
-- =============================================
CREATE PROCEDURE [wms].[BULK_DATA_SP_IMPORT_PRICE_LIST_BY_SKU_PACK_SCALE]
AS
BEGIN
  SET NOCOUNT ON;
  ...
  -- ------------------------------------------------------------------------------------
  TRUNCATE TABLE [SWIFT_EXPRESS].[wms].[SWIFT_PRICE_LIST_BY_SKU_PACK_SCALE]
  ...
  INSERT INTO #TEMP
    SELECT
     ...
     ,0
     ,'wms'
    FROM [SWIFT_INTERFACES_ONLINE].[wms].[ERP_PRICE_LIST_BY_SKU]
  ...
  INSERT INTO #TEMP
    SELECT
     ...
     ,0
     ,'wms'
    FROM [SWIFT_INTERFACES_ONLINE].[wms].[ERP_SKU_BASE_PRICE_BY_PACK]
  ...
  INSERT INTO #TEMP
    SELECT
     ...
     ,1
     ,'wms'
    FROM [SWIFT_INTERFACES_ONLINE].[wms].[ERP_PRICE_LIST_BY_SKU_PACK_SCALE]
  ...
  -- ------------------------------------------------------------------------------------
  INSERT INTO [SWIFT_EXPRESS].[wms].[SWIFT_PRICE_LIST_BY_SKU_PACK_SCALE] (CODE_PRICE_LIST
  , CODE_SKU
  , CODE_PACK_UNIT
  , [PRIORITY]
  , LOW_LIMIT
