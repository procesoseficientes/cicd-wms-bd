...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [wms].[BULK_DATA_SP_IMPORT_PACK_CONVERSION]
*/
-- =============================================
CREATE PROCEDURE [wms].[BULK_DATA_SP_IMPORT_PACK_CONVERSION]
AS
BEGIN
  SET NOCOUNT ON;
  --
  MERGE SWIFT_EXPRESS.wms.SONDA_PACK_UNIT AS TRG
  USING (SELECT
      epu.CODE_PACK_UNIT
     ...
     ,epu.LAST_UPDATE_BY
     ,epu.UM_ENTRY
    FROM SWIFT_INTERFACES_ONLINE.wms.ERP_PACK_UNIT epu) AS SRC
  ...
  USING (SELECT
     ...
     ,epc.LAST_UPDATE_BY
     ,epc.[ORDER]
    FROM SWIFT_INTERFACES_ONLINE.wms.ERP_PACK_CONVERSION epc) AS SRC
