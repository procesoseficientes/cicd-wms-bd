...
*/
-- =============================================
CREATE VIEW [wms].[ERP_VIEW_RECEPTION_DOCUMENT_DETAIL_SAE_PRUEBAS]
AS
SELECT *
FROM OPENQUERY
     ([ERP_SERVER],
...
FROM [SAE70EMPRESA01_PRUEBAS].[dbo].[COMPO04] [COMP]
    LEFT JOIN [SAE70EMPRESA01_PRUEBAS].[dbo].[PAR_COMPO04] [D]
        ON ([D].[CVE_DOC] = [COMP].[CVE_DOC])
    LEFT JOIN [SAE70EMPRESA01_PRUEBAS].[dbo].[PROV04] [PROV]
        ON [PROV].[CLAVE] = [COMP].[CVE_CLPV]
    LEFT JOIN [SAE70EMPRESA01_PRUEBAS].[dbo].[INVE04] [M]
