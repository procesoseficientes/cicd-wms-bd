...
--Modificacion 29-05-2018
					--christian.hernandez
					--Se modifico la insercion y actualizacion de INVENTARIO agregando el campo CODE_PACK_UNIT_STOCK colocando el valor "MANUAL" 

/*
-- Ejemplo de Ejecucion:
				EXEC [SONDA].[SWIFT_ERP_SP_BULK_DATA]
*/
-- =============================================
CREATE PROCEDURE [SONDA].[SWIFT_ERP_SP_BULK_DATA]
AS
PRINT 'INSERTA EN SKUs'
MERGE SONDA.SWIFT_ERP_SKU TRG
	USING (SELECT * FROM SWIFT_INTERFACES_ONLINE.SONDA.ERP_VIEW_SKU) AS SRC
	ON TRG.CODE_SKU = SRC.CODE_SKU AND TRG.LIST_NUM = SRC.LIST_NUM
WHEN MATCHED THEN 
	UPDATE 
	SET TRG.[SKU] = SRC.[SKU]
...
MERGE SONDA.SWIFT_ERP_SELLER TRG
...
MERGE SONDA.SWIFT_ERP_CUSTOMERS TRG
	USING (SELECT * FROM SWIFT_INTERFACES_ONLINE.SONDA.ERP_VIEW_COSTUMER) AS SRC
...
VALUES
	(SRC.[CODE_CUSTOMER]
      ...
      ,SRC.[NAME_ROUTE]
      ,SRC.[NAME_CLASSIFICATION]
	  ,ISNULL(SRC.[LATITUDE] + ',' + SRC.[LONGITUDE],'0,0')
...
DELETE FROM [SONDA].[SWIFT_ERP_RECEPTION]
INSERT INTO [SONDA].[SWIFT_ERP_RECEPTION]
SELECT EVR.SAP_RECEPTION_ID ,
       EVR.ERP_DOC ,
       EVR.PROVIDER_ID ,
       EVR.PROVIDER_NAME ,
       EVR.SKU ,
       EVR.SKU_DESCRIPTION ,
       EVR.QTY 
FROM [SWIFT_INTERFACES_ONLINE].[SONDA].[ERP_VIEW_RECEPTION] EVR
...
USING (SELECT * FROM  [SWIFT_INTERFACES_ONLINE].[SONDA].[ERP_PRICE_LIST_BY_SKU]) SRC 
ON  TGR.CODE_PRICE_LIST  = SRC.CODE_PRICE_LIST
...
ON
TGR.CODE_CUSTOMER = SRC.CODE_CUSTOMER 
WHEN MATCHED THEN 
UPDATE 
SET TGR.CODE_PRICE_LIST = SRC.CODE_PRICE_LIST 
WHEN NOT MATCHED THEN 
...
MERGE [SWIFT_INTERFACES].[SONDA].[SWIFT_ERP_WAREHOUSE] SWI 
USING ( SELECT * FROM  [SWIFT_INTERFACES_ONLINE].[SONDA].[ERP_VIEW_WAREHOUSE]) WVH 
ON
SWI.[CODE_WAREHOUSE] = WVH.[CODE_WAREHOUSE] 
WHEN MATCHED THEN 
UPDATE 
...
ON
(ISNULL(SIV.[SERIAL_NUMBER],'0') = CAST(ISNULL(EVI.[SERIAL_NUMBER],0) AS VARCHAR)  AND   SIV.[SKU] = EVI.[SKU]
AND EVI.LOCATION = SIV.LOCATION AND EVI.WAREHOUSE = SIV.WAREHOUSE) 
WHEN MATCHED THEN 
UPDATE 
SET   
...
ON
(SIW.[CODE_WAREHOUSE] = EVW.[CODE_WAREHOUSE] AND SIW.[CODE_SKU] = EVW.[CODE_SKU])
WHEN MATCHED THEN 
UPDATE 
